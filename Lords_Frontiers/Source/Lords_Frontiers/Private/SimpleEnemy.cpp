...
ASimpleEnemy::ASimpleEnemy()
{
	...
	CapsuleComponent->InitCapsuleSize( DefaultCapsuleRadius,
	                                   DefaultCapsuleHalfHeight ); // debug size
...
#if WITH_EDITOR
	// Draw a small debug sphere so you can see the spawn in editor/game for a
	// second
	if ( GetWorld() )
	{
...
void ASimpleEnemy::HandleDeath()
{
	...
	{
		FTimerHandle Th;
		GetWorld()->GetTimerManager().SetTimer( Th, [this]() { Destroy(); }, 0.1f, false );
	}
	else
	{
		Destroy();

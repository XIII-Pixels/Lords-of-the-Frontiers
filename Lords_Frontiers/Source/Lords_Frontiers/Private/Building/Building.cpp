...
void ABuilding::TakeDamage( float damage )
{
	...
	// ≈сли кеша нет или он невалиден Ч попробуем закешировать заново (фоллбэк)
	if ( !IsValid( Mgr ) )
	{
	...
	else
	{
		UE_LOG(
		    LogTemp, Warning,
		    TEXT( "ABuilding::TakeDamage: HealthBarManager not available for actor %s Ч skipping UI update" ),
		    *GetNameSafe( this )
		);
	}

...
FWave::FWave () 
	...
	, SpawnPointIds () 
	, StartDelay ( 0.0f ) 
	, GroupSpawnDelay ( 0.0f ) 
{
}

FWave::FWave ( 
	const TArray <FEnemyGroup> & inGroups,
	const TArray <TSoftObjectPtr <AEnemyGroupSpawnPoint> > & inRefs,
	const TArray <FName> & inIds,
	float inStartDelay,
	float inGroupSpawnDelay ) 
	: EnemyGroups ( inGroups ) 
	, SpawnPointReferences ( inRefs ) 
	, SpawnPointIds ( inIds ) 
	, StartDelay ( FMath::Max ( 0.0f, inStartDelay ) ) 
	, GroupSpawnDelay ( FMath::Max ( 0.0f, inGroupSpawnDelay ) ) 
{
}
...
FTransform FWave::GetSpawnTransformForGroup ( UObject* worldContextObject, int32 groupIndex ) const
{
	...
	if  ( SpawnPointIds.IsValidIndex ( groupIndex ) ) 
	{
		...
		{
			if  ( AEnemyGroupSpawnPoint* foundSpawnPoint = AEnemyGroupSpawnPoint::FindSpawnPointById ( worldContextObject, spawnPointId ) ) 
			{

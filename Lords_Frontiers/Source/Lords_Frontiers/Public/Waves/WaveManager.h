...
public:
	...
	// --- Infinite mode -----------------------------------------------------
	// Optional: when set and CurrentWaveIndex >= InfiniteConfig->StartWaveIndex,
	// waves are procedurally generated by the infinite mode builder. Regular
	// waves (index < StartWaveIndex) keep coming from UWaveConfigData.
	...
	// Deterministic seed for the infinite builder. 0 = use random seed at
	// BeginPlay.
	UPROPERTY( EditAnywhere, BlueprintReadWrite, Category = "Settings|InfiniteMode" )
...
private:
	...
	// Lazily build (or rebuild) the infinite builder using current InfiniteConfig.
	void EnsureInfiniteBuilder();

	// Build & cache a UWaveData for an infinite-mode wave index.
	// Returns the resulting (transient) UWaveData or nullptr.

...
* (Artyom)
  ...
  - Has optional SpawnPointId (FName) that Wave / WaveManager can use to lookup the point
...
public:
	...
	// Unique identifier for this spawn point. Can be used by Wave/WaveManager to find the point
	...
	// Visual components (arrow + billboard) to help designer see orientation in editor
...
public:
	...
	UFUNCTION ( BlueprintPure, Category = "Spawn" )
	FVector GetSpawnForwardVector () const { return GetActorForwardVector (); }

	// Does this spawn point match the given id/name? (checks SpawnPointId and optionally ActorTags)
	UFUNCTION ( BlueprintCallable, Category = "Spawn" )
	bool MatchesId ( const FName& id ) const;
	
	// Helper: find first spawn point in world that matches id
	// Returns nullptr if none found. Useful for WaveManager: pass GetWorld() as WorldContext
	...
	// Helper: find all spawn points that match id (may be multiple when using tags)
	UFUNCTION ( BlueprintCallable, Category = "Spawn", meta = (WorldContext = "WorldContextObject" ) )
	static void FindAllSpawnPointsById ( UObject* worldContextObject, const FName& id, TArray<AEnemyGroupSpawnPoint*>& outFound );
};